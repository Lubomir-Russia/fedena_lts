...
class ReminderController < ApplicationController
  ...
  def index
    @user = current_user
    @reminders = Reminder.paginate(:page => params[:page], :conditions => ['recipient = ? AND is_deleted_by_recipient = ?', @user.id, false], :order => 'created_at DESC', :include => :user)
    @read_reminders = Reminder.find_all_by_recipient_and_is_read_and_is_deleted_by_recipient(@user.id, true, false, :order => 'created_at DESC')
    @new_reminder_count = Reminder.find_all_by_recipient_and_is_read_and_is_deleted_by_recipient(@user.id, false, false)
  end

  def create_reminder
    @user = current_user
    @departments = EmployeeDepartment.all
    @new_reminder_count = Reminder.find_all_by_recipient_and_is_read(@user.id, false)
    ...
    if request.post?
      if params[:reminder][:body].present? && params[:recipients].present?
  ...
  def select_employee_department
    @user = current_user
    @departments = EmployeeDepartment.find_all_by_status(true)
    render :partial => 'select_employee_department'
  end

  def select_users
    @user = current_user
    users = User.active.find_all_by_student(false)
  ...
  def to_employees
    if params[:dept_id].present?
      ...
      @to_users = employees.map { |s| s.user.id if s.user.present? }
      @to_users.delete nil
  ...
  def sent_reminder
    @user = current_user
    @sent_reminders = Reminder.paginate(:page => params[:page], :conditions=>['sender = ? AND is_deleted_by_sender = ?', @user.id, false],  :order => 'created_at DESC')
    @new_reminder_count = Reminder.find_all_by_recipient_and_is_read(@user.id, false)
  end
  ...
  def send_reminder
    if params[:create_reminder]
      if params[:create_reminder][:message].present? && params[:create_reminder][:to].present?
        ...
        render(:update) do |page|
          page.replace_html 'error-msg', :text => "<p class='flash-msg'>#{I18n.t('your_message_sent')}</p>"
        end
      else
        render(:update) do |page|
          page.replace_html 'error-msg', :text=> "<p class='flash-msg'>#{I18n.t('enter_subject')}</p>"
        end
      end
    else
  ...
  def reminder_actions
    ...
    @reminders = Reminder.paginate(:page => params[:page], :conditions => ['recipient = ? AND is_deleted_by_recipient = ?', @user.id, false], :order => 'created_at DESC')
    @new_reminder_count = Reminder.find_all_by_recipient_and_is_read_and_is_deleted_by_recipient(@user.id, false, false)
