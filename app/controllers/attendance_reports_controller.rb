...
class AttendanceReportsController < ApplicationController
  ...
  def subject
    ...
    if @current_user.employee? && @allow_access
      role_symb = @current_user.role_symbols

      if role_symb.include?(:student_attendance_view) || role_symb.include?(:student_attendance_register)
        @subjects = Subject.all(:conditions => {:batch_id => @batch.id})
      else
    ...
    if @config.config_value != 'Daily'
      if @mode == 'Overall'
        ...
        if params[:subject_id] != '0'
          ...
          @report = @batch.subject_leaves.all(:conditions => {:batch_id => @batch.id, :month_date => @start_date..@end_date})
  ...
  def report2
    ...
    if @start_date <= @local_tzone_time.to_date
      ...
      if @config.config_value == 'Daily'
        @report = @batch.attendances.all(:conditions => {:month_date => @start_date..@end_date})
      else
  ...
  def report
    ...
    if @start_date < @batch.started_on.beginning_of_month || @start_date > @batch.ended_on || @start_date >= @today.next_month.beginning_of_month
      render :update do |page|
        page.replace_html 'report', :text => I18n.t('no_reports')
      end
    else
      @leaves = Hash.new { |h, k| h[k] = Hash.new(&h.default_proc) }
      working_days = @batch.working_days(@date.to_date)
      if @start_date <= @local_tzone_time.to_date
        ...
        if @config.config_value == 'Daily'
          ...
          if params[:subject_id] != '0'
            ...
            @academic_days = @batch.subject_hours(@start_date, @end_date, 0).values.flatten.compact.count
            @report = @batch.subject_leaves.all(:conditions => {:month_date => @start_date..@end_date})
  ...
  def student_details
    ...
    @config = Configuration.find_by_config_key('StudentAttendanceType')
    if @config.config_value == 'Daily'
      @report = Attendance.all(:conditions => {:student_id => @student.id, :batch_id => @batch.id})
    else
      @report = SubjectLeave.all(:conditions => {:student_id => @student.id, :batch_id => @batch.id})
    end
  end
