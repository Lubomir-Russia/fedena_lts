...
class FaGroupsController < ApplicationController
  ...
  def update_subject_fa_groups
    ...
    new_fa_groups = params[:subject][:fa_group_ids] if params[:subject].present?
    new_fa_groups ||= []
  ...
  def create_fa_criteria
    ...
    @fa_criteria.sort_order = @fa_group.fa_criterias.last(:order => 'sort_order ASC').try(:sort_order).to_i + 1 || 1
    if @fa_criteria.save
      @fa_criterias = @fa_group.fa_criterias.active
  ...
  def update_fa_criteria
    @fa_criteria = FaCriteria.find(params[:id])
    if @fa_criteria.update_attributes(params[:fa_criteria])
  ...
  def destroy_fa_criteria
    ...
    @fa_criterias = @fa_criteria.fa_group.fa_criterias.active
    render(:update) do |page|
      page.replace_html 'flash-box', :text => "<p class='flash-msg'>#{flash[:notice]}</p>" if flash[:notice].present?
