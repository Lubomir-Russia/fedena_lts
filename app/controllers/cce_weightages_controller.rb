...
class CceWeightagesController < ApplicationController
  ...
  def update
    @weightage = CceWeightage.find(params[:id])
    if @weightage.update_attributes(params[:cce_weightage])
  ...
  def assign_courses
    ...
    if request.post?
      new_courses = params[:cce_weightage] ? params[:cce_weightage][:course_ids] : []
  ...
  def update_course_weightages
    ...
    new_weightages = params[:course] ? params[:course][:weightage_ids] : []
