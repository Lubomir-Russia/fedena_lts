...
class AttendancesController < ApplicationController
  ...
  def index
    ...
    if current_user.admin? || @current_user.privileges.map{|p| p.name}.include?('StudentAttendanceRegister')
      ...
      if @config.config_value == 'Daily'
        ...
        @batches = @batches.uniq if @batches.any?
      end
    end
  end
  ...
  def list_subject
    if params[:batch_id].present?
      ...
      if @current_user.employee? && @allow_access && !@current_user.privileges.map{|m| m.name}.include?("StudentAttendanceRegister")
        if @batch.employee_id.to_i == @current_user.employee_record.id
          @subjects = @batch.subjects
        else
          @subjects = Subject.all(:joins => ["INNER JOIN employees_subjects ON employees_subjects.subject_id = subjects.id AND employee_id = ? AND batch_id = ?", @current_user.employee_record.id, @batch.id])
        end
      end
  ...
  def subject_wise_register
    if params[:subject_id].present?
      ...
      if @sub.elective_group_id.present?
        elective_student_ids = StudentsSubject.find_all_by_subject_id(@sub.id).map { |x| x.student_id }
        @students = @batch.students.all(:conditions => ["FIND_IN_SET(id, ?)", elective_student_ids.split.join(',')])
      else
      ...
      @students.each do |student|
        ...
        if subject_leaves[student.id].present?
  ...
  def daily_register
    ...
    @students.each do |student|
      @leaves[student.id] = Hash.new(false)
      if attendances[student.id].present?
  ...
  def create
    ...
    respond_to do |format|
      if @absentee.save
        ...
        if sms_setting.application_sms_active && @student.is_sms_enabled && sms_setting.attendance_sms_active
          recipients = []
          if @config.config_value != "SubjectWise"
          ...
          if sms_setting.student_sms_active
            recipients << @student.phone2 if @student.phone2.present?
          end
          if sms_setting.parent_sms_active && @student.immediate_contact_id.present?
            guardian = Guardian.find(@student.immediate_contact_id)
            recipients << guardian.mobile_phone if guardian.mobile_phone.present?
          end
          if recipients.any?
  ...
  def update
    ...
    if @config.config_value == 'Daily'
      @absentee = Attendance.find params[:id]
      @student = Student.find(@absentee.student_id)
      @error = true unless @absentee.update_attributes(params[:attendance])
    else
      @absentee = SubjectLeave.find params[:id]
      @student = Student.find(@absentee.student_id)
      @error = true unless @absentee.update_attributes(params[:subject_leave])
    end
    respond_to do |format|
  ...
  def only_privileged_employee_allowed
    ...
    if @current_user.employee?
      ...
      if @employee_subjects.empty? && !@privilege.include?("StudentAttendanceRegister")
