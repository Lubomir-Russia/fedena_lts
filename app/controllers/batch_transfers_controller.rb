...
class BatchTransfersController < ApplicationController
  ...
  def transfer
    if request.post?
      ...
      if params[:transfer][:to].present?
        if params[:transfer][:students].present?
          ...
          students.each do |s|
            ...
            s.update_attribute(:has_paid_fees, false)
          end
        end
        batch = @batch
  ...
  def graduation
    ...
    if request.post?
      ...
      @student_list.each do |s|
        s.archive_student(params[:graduate][:status_description])
      end
      @stu = Student.find_all_by_batch_id(@batch.id)
      if @stu.empty?
        @batch.update_attribute :is_active, false
  ...
  def get_previous_batch_subjects
    ...
    @previous_batch = Batch.first(:order => 'id desc', :conditions => ["batches.id < ? AND batches.is_deleted = ? AND course_id = ?", @batch.id, false, course_id], :joins => "INNER JOIN subjects ON subjects.batch_id = batches.id  AND subjects.is_deleted = 0")
    if @previous_batch.present?
      ...
      render(:update) do |page|
        page.replace_html 'msg', :text => "<p class='flash-msg'>#{I18n.t('batch_transfers.flash4')}</p>"
      end
    end
  end
  ...
  def assign_previous_batch_subject
    ...
    if sub_exists.empty?
      ...
      render(:update) do |page|
        ...
        page.replace_html "errors", :text => "#{subject.name}  #{I18n.t('has_been_added_to_batch')}:#{batch.name}"
      end
    else
      render(:update) do |page|
        page.replace_html "prev-subject-name-#{subject.id}", :text => ""
        page.replace_html "errors", :text => "<div class=\"errorExplanation\"><p>#{batch.name} #{I18n.t('already_has_subject')} #{subject.name}</p></div>"
      end
    end
  end
  ...
  def assign_all_previous_batch_subjects
    ...
    @previous_batch_electives = Subject.find_all_by_batch_id(@previous_batch.id, :conditions => ["elective_group_id IS NOT NULL AND is_deleted = false"])
    render(:update) do |page|
      page.replace_html 'previous-batch-subjects', :text => "<p>#{I18n.t('subjects_assigned')}</p> "
      if msg.present?
        page.replace_html "msg", :text => "<div class=\"flash-msg\"><ul>" + msg +"</ul></div>"
      end
      if err.present?
        page.replace_html "errors", :text => "<div class=\"errorExplanation\" ><p>#{I18n.t('following_errors_found')} :</p><ul>" + err + "</ul></div>"
      end
    end
  end
  ...
  def new_subject
    @subject = Subject.new
    @batch = Batch.find params[:id] if request.xhr? && params[:id]
    @elective_group = ElectiveGroup.find params[:id2] if params[:id2].present?
  ...
  def create_subject
    ...
    @batch = @subject.batch
    if @subject.save
      @normal_subjects = @subject.batch.normal_batch_subject
